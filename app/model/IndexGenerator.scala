...
class IndexGenerator(s3client: AWSS3, backReference: String) {

  val footer = Html("""Generated by <a href="%s">S3Index</a>""".format(backReference))

  def generate(properties: Properties, prefix: String = "", marker: String = ""): Html = {
  ...
  def html(bucketName: String, prefix: String, keys: Seq[S3ObjectSummary], prefexes: Seq[String], template: Template, filesFormat: FilesListFormat, next: String, current: String): Html = {
    ...
    template match {
      case Simple =>
        views.html.templates.simple(if (prefix.isEmpty()) "/" else "/" + prefix, data, prevHtml, nextHtml, footer, backReference)
      case Slim =>
        views.html.templates.slim(if (prefix.isEmpty()) "/" else "/" + prefix, data, prevHtml, nextHtml, footer, backReference)
    }
  }

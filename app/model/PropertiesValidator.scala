...
class PropertiesValidator(properties: Map[String, Seq[String]], errors: List[Map[String, String]] = Nil) {
  ...
  def isNumberInRange(key: String, range: Range): PropertiesValidator = {
    val message = "The value of this parameter should be greater than " + (range.start - 1) + " and less than " + (range.end + 1) + "."
    try {
      validateWith(key, v => !v.isEmpty() && v.forall(_.isDigit) && range.contains(v.toInt), message)
    } catch {
      case e: NumberFormatException => new PropertiesValidator(this.properties, newError(key, message) :: errors)
    }
  }

  def isNumber(key: String): PropertiesValidator = {
    val message = "The value of this parameter should be a number."
    try {
      validateWith(key, v => v.forall(_.isDigit), message)
    } catch {
      case e: NumberFormatException => new PropertiesValidator(this.properties, newError(key, message) :: errors)
    }
  }
  ...
  def isLengthInRange(key: String, range: Range): PropertiesValidator = {
    validateWith(key, v => range.contains(v.length()), "The length of this parameter should be greater than " + (range.start - 1) + " and less than " + (range.end + 1) + ".")
  }
